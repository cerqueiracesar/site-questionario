document.addEventListener("DOMContentLoaded", () => {
  ...
  function verificarDisponibilidade() {
      ...
      if (agora >= dataLiberacao || feedbackDisponivel) {
          // Marcar como disponível no localStorage (só se ainda não estiver marcado)
  ...
  if (!verificarDisponibilidade()) {
      ...
      feedbackSection.classList.add("locked"); // Garante que a seção comece bloqueada
      atualizarContador();

      // Atualizar o contador a cada segundo
      intervalId = setInterval(atualizarContador, 1000);
  } else {
      // Se já estiver disponível ao carregar a página
      feedbackSection.classList.remove("locked");
      feedbackSection.classList.add("available");
      feedbackButton.disabled = false;
      feedbackCountdown.innerHTML =
